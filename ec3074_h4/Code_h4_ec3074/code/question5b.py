...
fp = open("output/tag_dev5b.out", "w")
...
def ComputeFeatures(enum_histories, sentence):
  ...
  for item in history_list:
    ...
    if(len(elements) == 3):  
      #g[(elements[0], elements[1], elements[2])] = 1
      weight1 = float(v[(elements[1], elements[2])])  #compute weight for each history
      
      #g[(elements[0], word_list[int(elements[0])-1], elements[2])] = 1
      ...
      weight[(elements[0], elements[1], elements[2])] = weight1 + weight2 + weight3
...
if __name__ == '__main__':
  initializeWeights("tag.model", "output/suffix_tagger.model")
