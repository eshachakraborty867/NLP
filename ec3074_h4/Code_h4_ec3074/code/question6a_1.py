	...
	for item in history_list:
		...
		if len(elements)==3:
			...
			if len(tempstring[0])==1:
				weight=v[(tempstring[0][-1:], elements[2])] + v[(tempstring[0], elements[2])] + v[(elements[1], elements[2])] + tempweight1
				hist=hist+item+" "+str(weight)+"\n"
			elif len(tempstring[0])==2:
				weight=v[(tempstring[0][-2:], elements[2])] + v[(tempstring[0][-1:], elements[2])] + v[(tempstring[0], elements[2])] + v[(elements[1], elements[2])] + tempweight1
				hist=hist+item+" "+str(weight)+"\n"
			else:
				weight = v[(tempstring[0][-1:], elements[2])] + v[(tempstring[0][-2:], elements[2])] + v[(tempstring[0][-3:], elements[2])] + v[(tempstring[0], elements[2])] + v[(elements[1], elements[2])] + tempweight1
				hist=hist+item+" "+str(weight)+"\n"
...
if __name__ == '__main__':
	...
	for line in trainlines:
		if line == "\n":
			N+=1
	fp1.close()
	fp = open("output/suffix_tagger6_1.model", "w")
