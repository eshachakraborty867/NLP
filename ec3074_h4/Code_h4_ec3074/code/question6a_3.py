...
def ComputeFeatures(enum_histories, sentence, gold_history,t):
	...
	if decoder_history != gold_history:
		...
		for i in xrange(len(gold_list)):
			...
			#current tag - current word - next word
			if i+1 < len(words)-1:
				next_word_list1 = words[i+1].strip().split()
				f_gold[(word_list[0], next_word_list1[0], gold_list_elements[2])] += 1
				f_decoder[(word_list[0], next_word_list1[0], decoder_list_elements[2])] += 1
				f_decoder[(word_list[0], next_word_list1[0], gold_list_elements[2])] += 0
				f_gold[(word_list[0], next_word_list1[0], decoder_list_elements[2])] += 0
...
if __name__ == '__main__':
	...
	for line in trainlines:
		if line == "\n":
			N+=1
	fp1.close()
	fp = open("output/suffix_tagger6_3.model", "w")
